  ...
  async getComercials({ query }) {
    return new Promise((resolve, reject) => {
      this.comercials.findMany({
        ...
        return resolve({ code: 200, message: result.length == 0 ? "No se encontraron comerciantes." : "Comerciantes con éxito", total: maxCount, hasMore: (query.start || 0) + (query.end || maxCount) < maxCount, comercials: result });
      }).catch((error) => {
        console.log(error);
