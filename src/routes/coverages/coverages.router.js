...
router
  ...
  .get((req, res) => {
    controller
      .getCoverage({ search: req.params.id, query: req.query })
      .then((r) => res.status(r.code).send(r))
      .catch((c) => res.status(c.code).send(c));
  })
  .delete((req, res) => {
    controller
      .deleteCoverage(req.params.id)
      .then((r) => res.status(r.code).send(r))
      .catch((c) => res.status(c.code).send(c));
  })
  .put(
    ...
    (req, res) => {
      controller
        .updateCoverage({ search: req.params.id, data: req.body, query: req.query })
        .then((r) => res.status(r.code).send(r))
        .catch((c) => {
          return res.status(c.code).send(c);
        });
