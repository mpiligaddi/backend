  ...
  async updateZone({ search, data, query }) {
    return new Promise((resolve, reject) => {
      this.zones.update({
        ...
        if (!result) return reject({ code: 200, message: "No se encontró la zona" })
  ...
  async getZones({ query }) {
    return new Promise((resolve, reject) => {
      this.zones.findMany({
        ...
        return resolve({ code: 200, message: result.length == 0 ? "No se encontraron las zonas." : "Zonas encontradas con éxito", total: maxCount, hasMore: (query.start || 0) + (query.end || maxCount) < maxCount, zones: result });
      }).catch((error) => {
        console.log(error);
