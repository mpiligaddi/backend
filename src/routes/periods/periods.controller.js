  ...
  async updatePeriod({ search, data, query }) {
    return new Promise((resolve, reject) => {
      this.periods.update({
        ...
        if (!result) return reject({ code: 200, message: "No se encontró al periodo" })
  ...
  async getPeriods({ query }) {
    return new Promise((resolve, reject) => {
      ...
      this.periods.findMany({
        ...
        if (result.length == 0) return reject({ code: 200, message: "No se encontraron periodos.", periods: [] });
