...
router.route("/products")
  ...
  .get((req, res) => {
    controller.getProducts({ query: req.query })
      .then((r) => res.status(r.code).send(r))
      .catch((c) => {
        console.log(c);
...
router.route("/products/:id")
  .get((req, res) => {
    controller.getProduct({ search: req.params.id, query: req.query })
      .then((r) => res.status(r.code).send(r))
  ...
  .put([
    ...
    controller.updateClient({ search: req.params.id, data: req.body })
      .then((r) => res.status(r.code).send(r))
      .catch((c) => {
        return res.status(c.code).send(c)
      })
  ...
  .patch((req, res) => {
    controller.updateProduct({ search: req.params.id, data: req.body })
      .then((r) => res.status(r.code).send(r))
      .catch((c) => {
        return res.status(c.code).send(c)
      })
